 ...
 * @param slippage 滑点 double或string
...
async function addOrder(coinPair, orderType, price, volume, maxWaitSeconds, gasPriceGwei, slippage) {
    ...
    console.info(`${logHead(beginTime)}应该输出${trade.outputAmount.toFixed(6)},不得低于${trade.minimumAmountOut(util.doubleToPersent(slippage)).toFixed(6)}`)
    //2.发送交易 .确定该调用uniswapV2Router02的哪个方法
    let allowedSlippage = slippage && slippage > 0 ? util.doubleToPersent(slippage) : defaultSlipage
    //let allowedSlippage = slippage && slippage > 0 ? util.doubleToPersent(0.03) : defaultSlipage
    ...
    console.info(`${logHead(beginTime)}fetchPairData，耗时：${Date.now() - beginTime},methodName:${swapParameters.methodName},value:${swapParameters.value}，slippage:${allowedSlippage.toFixed(3)}%,swapParameters:${JSON.stringify(swapParameters)}`)
    /* overrides参数：
    ...
    let gasLimit
    if (swapParameters.methodName === 'swapExactETHForTokens') {
        gasLimit = 140000
    } else if (swapParameters.methodName === 'swapExactTokensForETH') {
        gasLimit = 155000
    } else {
        gasLimit = 200000
    }
...
async function sendAndWait(uniswapV2Router02, swapParameters, gasPriceGwei, gasLimit, beginTime, transId, maxWaitSeconds, nonce) {
    ...
    if (packSucceed) {
        ...
        https.get(
            ...
            transactionResponse.hash + '&apikey=' + config.etherscanAPIKey,
            ...
            res => {
                res.on('data', (d) => {
                    ...
                    if (result.status === '1') {//如果接口调用成功
                        //console.log(logHead(transId) + " 交易哈希:" + transactionResponse.hash + ",交易状态：" + result.result.isError === '1' ? '失败' : '成功' + ":" + result.result.errDescription)
                    }
                });
            }
...
//估算limit
async function gasLimit_addOrder(coinPair, orderType, price, volume, maxWaitSeconds, gasPriceGwei, slippage) {
    let beginTime = new Date().getTime()
...
exports.gasLimit_addOrder = gasLimit_addOrder
