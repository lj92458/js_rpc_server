...
import TickLensABI from '@uniswap/v3-periphery/artifacts/contracts/lens/TickLens.sol/TickLens.json'
...
export async function getPoolInfo(provider, tokenA, tokenB, poolFee) {
    ...
    const poolContract = new ethers.Contract(Pool.getAddress(tokenA, tokenB, poolFee), IUniswapV3PoolABI.abi, provider)
...
export async function getPool(provider, tokenA, tokenB, poolFee) {
    ...
    return new Pool(poolInfo.token0, poolInfo.token1, poolInfo.fee, poolInfo.sqrtPriceX96, poolInfo.liquidity, poolInfo.currentTick)
}
...
export async function creatPoolWithticksFromPool(pool, bytes) {
    ...
    */
    ...
    let address = Pool.getAddress(pool.token0, pool.token1, pool.fee)
    ...
    if (queryTicksCount++ % space === 0 || currentByteIndex !== currentByteIndexCache || bytesCache !== bytes || addressCache !== address) {
        ...
        tickDataProviderCache = new TickListDataProvider(tickArr, pool.tickSpacing)
        bytesCache = bytes
...
async function getPopulatedTicksInWord(address, byteIndex) {
    const tickLensContract = new ethers.Contract(constant.tickLens, TickLensABI.abi, config.provider)
