 ...
 * @param r {Number} r值，右叫orderStepRatio.也就是两个相邻挂单之间价格递增的幅度，默认r= 0.05%+0.02%
 * @return {Promise<Pool>}
 */
 ...
 */
async function createOrderBook(pool, goodsToken, moneyToken, marketOrderSize, orderStepRatio, poolFee) {
    //用卖的办法(输入goods)，模拟出市场买单。然后我可以提交卖单吃掉这些市场买单。
    bidsCache = await createMarketOrder(pool, goodsToken, moneyToken, marketOrderSize, orderStepRatio, goodsToken, poolFee)
    //用买的办法(输入money)，模拟出市场卖单。然后我可以提交买单吃掉这些市场卖单。
    asksCache = await createMarketOrder(pool, moneyToken, goodsToken, marketOrderSize, orderStepRatio, goodsToken, poolFee)
}
