...
export async function initPools() {
    ...
    //定时查询ticks
    let timeSpace = 4000 //每3秒提交一个ticks查询请求
    setInterval(() => {
        let i = 0
        for (let key in poolMap) {
            setTimeout(() => {
                try {
                    burnMintProcess(poolMap[key]);
                } catch (e) {
...
async function updateAndTrade() {
    ...
    if (updateAndTrading) {
        ...
        try {
            ...
            await autoTradeMultiMoney()
            //console.log(`${beginTime} updatePools耗时${Date.now()-beginTime}毫秒`)
        } catch (e) {
    ...
    if (poolInfo2Arr.length > 0) {
        ...
        for (let i in poolInfo2Arr) {
            ...
            if (poolInfo2.currentByteIndex !== currentByteIndex) {
                await fetchTicks(poolInfo2.marketOrderSize, poolInfo, poolInfo2.r)
            }
...
export async function fetchTicks(marketOrderSize, pool, r) {
    ...
    if (!poolMap[poolAddress].fetchTickRunning) {
        poolMap[poolAddress].fetchTickRunning = true
        try {
            ...
            if (tickArr.length === 0) {
                throw `${pool.token0.symbol}-${pool.token1.symbol}异常：tickArr为空，`
            }
            ...
            console.log('fetchTicks异常：', e)
            //await fetchTicks(marketOrderSize, pool, r)
        } finally {
            poolMap[poolAddress].fetchTickRunning = false
        }
    }
...
async function autoTradeMultiMoney() {
    ...
    for (let autoTradeAmount of autoTradeAmountArr) {
        ...
        let transRouteArr2 = [] //数组每个元素是{poolIndexArr, profit, initInputAmount,amountOut}
    ...
    let bestRoute = bestRouteArr.sort((o1, o2) => o2.profit - o1.profit)[0]
    //console.info(new Date() + `路由${bestRoute.poolIndexArr},本机预测利润${bestRoute.profit}usdt`)
    if (bestRoute.profit >= atleastEarn) {
        if (false) {//todo 有了预查询，可还是落空了。是否是因为预查询浪费了宝贵的时间，导致别人抢先？那就不要预查询
            ...
            if (quoteProfit >= atleastEarn) {
                console.log(new Date() + `套利正式开始：路由${bestRoute.poolIndexArr},本机预测利润${bestRoute.profit}，线上预查询利润${quoteProfit},交易额${bestRoute.initInputAmount.toFixed()}`)
                await transRoute(bestRoute.poolIndexArr, bestRoute.initInputAmount, bestRoute.amountOut, quoteProfit, false)
            }
        } else {
            console.log(new Date() + `套利正式开始：路由${bestRoute.poolIndexArr},本机预测利润${bestRoute.profit}，交易额${bestRoute.initInputAmount.toFixed()}`)
...
async function processOneRoute(amountIn, poolIndexArr) {
    ...
    return {poolIndexArr, profit: Number(profit), initInputAmount, amountOut}
}
...
export async function transRoute(poolIndexArr, inputAmount, outputAmount, profit, isQuote) {
    ...
    if (isQuote) {
        ...
        const methodParameters = SwapRouter.swapCallParameters([trade], {
            slippageTolerance: doubleToPersent(0.98 * profit / inputAmount.toFixed()),
