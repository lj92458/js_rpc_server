...
export async function getPool(provider, tokenA, tokenB, poolFee, marketOrderSize = 0, r = 0.0007) {
    ...
    if (!poolCache || !poolCache.token0.equals(token0) || !poolCache.token1.equals(token1) || poolCache.fee !== poolFee) {
        ...
        poolCache = new Pool(tokenA, tokenB, poolFee, initPoolInfo.sqrtRatioX96, initPoolInfo.liquidity, initPoolInfo.tickCurrent, tickDataProviderCache)
        ...
        provider.on(
            ...
            async (log) => {//log数据结构：https://docs.ethers.org/v5/api/providers/types/#providers-Log
                if (log.topics[0] === swapFilter.topics[0]) {//Swap事件，导致价格发生变化，可能要重新获取ticks
                    ...
                    poolCache = new Pool(tokenA, tokenB, poolFee, currentPoolInfo.sqrtRatioX96, currentPoolInfo.liquidity, currentPoolInfo.tickCurrent, tickDataProviderCache)
                    ...
                    if (Date.now() - queryTicksTime > provider.pollingInterval) {//queryTicks返回数据很多，很耗资源，因此不轻易启动
                        ...
                        if (poolCache) {
                            poolCache = new Pool(tokenA, tokenB, poolFee, poolCache.sqrtRatioX96, poolCache.liquidity, poolCache.tickCurrent, tickDataProviderCache)
                        }
                    }
                }
