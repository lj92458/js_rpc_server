...
import {CallContract, callProvider, provider} from '../config.js'
...
let queryPoolTime = 0 //最后一次查询pool的时间
let poolCache = null
 ...
 * @param fromCache{boolean} 一定要从cache中读取吗？
 * @return {Promise<Pool>}
 */
export async function getPool(provider, tokenA, tokenB, poolFee, fromCache) {
    let [token0, token1] = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA]
    //3秒之内不再重复查询。如果坚持要求读取cache，也不用查询
    if (!fromCache && (Date.now() - queryPoolTime > 3000 || !poolCache || !poolCache.token0.equals(token0) || !poolCache.token1.equals(token1))) {
        let poolInfo = await getPoolInfo(provider, token0, token1, poolFee)
        poolCache = new Pool(token0, token1, poolInfo.fee, poolInfo.sqrtPriceX96, poolInfo.liquidity, poolInfo.currentTick)
        queryPoolTime = Date.now()
    }
    return poolCache
}
    ...
    */
    ...
    if (queryTicksCount++ % space === 0 || currentByteIndex !== currentByteIndexCache || addressCache !== address) {
        ...
        for (let i = 0; i < bytes; i++) {
            promiseArr.push(tickLensContract.getPopulatedTicksInWord(address, currentByteIndex + i + 1), tickLensContract.getPopulatedTicksInWord(address, currentByteIndex - i - 1))
        }//end for
