...
export async function getPool(provider, tokenA, tokenB, poolFee, fromCache) {
    ...
    if (!fromCache && (Date.now() - queryPoolTime > 3000 || !poolCache)) {
        let poolInfo = await getPoolInfo(provider, tokenA, tokenB, poolFee)
        poolCache = new Pool(tokenA, tokenB, poolInfo.fee, poolInfo.sqrtPriceX96, poolInfo.liquidity, poolInfo.currentTick)
