import sdkCore, {SupportedChainId} from '@uniswap/sdk-core'
import IERC20
...
export const pools = [//注意：不要调整数组中各元素的位置，否则下面routes会失效！！！
    ...
    {id: 3, goods: 'wbtc', money: 'usdt', fee: 3000, address: null},//500的池子出故障了。因此换成3000的
    ...
    // 下面是小币种，可能需要放在另一个app实例上运行。需要用不同的apiKey.
...
export const autoTradeInSymbol = 'usdt'//自动套利时的输入资金。要确保SwapRouter合约有权限花费我的这种币（不论中间经历了多少种币，我的账户需要支出的币始终只有一种）
 ...
 * 每个pool都会先给出我想要的币，在问我要另一种币。在这两步之间，pool还会调用我指定的函数，用来支付给pool另一种币。如果我指定的函数是调用下一个pool，那么实际执行顺序为：
 * 3.1 pool3借给我usdt
 * 2.1 pool2偿还pool3 wbtc
 * 1.1 pool1偿还pool2 eth
 * 1.11 我偿还pool1 usdt.
 * 1.2 pool1检测自己是否收到usdt(已收到，谁给的都无所谓)
 * 2.2 pool2检测自己是否收到eth(已收到，谁给的都无所谓)
 * 3.2 pool3检测自己是否收到wbtc(已收到，谁给的都无所谓)
 * 不难看出：在3.1和3.2之间，夹杂着其它步骤，下一个池主动偿还了上一个池的债务，形成连锁反应。
 * 从上到下依次执行，pool3先给我usdt，我再向pool1偿还usdt。所以我的账户上不需要有钱。不论中间经历了多少种币，我的账户需要支出的币始终只有一种
 */
export const autoTradeAmountArr = [
    1000, 2000, 3000, 5000, 8000, //几千
    //10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, //几万
    //100000, 150000, 200000, 300000, 500000, 800000, //几十万
    //1000000, 2000000, 3000000, //几百万
]
export const atleastEarn = 5 //10
