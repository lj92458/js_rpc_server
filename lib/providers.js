...
import {logHead} from "../util";

//const sdkCore = require('@uniswap/sdk-core')
const config = require('../config')

const mainnetProvider = config.provider
export const wallet = config.initWallet(config.provider)
...
export function getWalletAddress() {
    return config.env === Environment.WALLET_EXTENSION
 ...
 * @param transactionRequest {ethers.TransactionRequest}
 * @returns {Promise<string>}
 */
export async function sendTransaction(transactionRequest) {
    if (config.env === Environment.WALLET_EXTENSION) {
...
{
    try {
        const receipt = await config.provider?.send(
            'eth_sendTransaction',
            [transactionRequest]
        )
        if (receipt) {
...
export async function sendTransactionByWallet(transactionRequest, maxWaitSeconds, gasPriceGwei) {
    ...
    if (!packSucceed) {
        [packSucceed, transactionResponse, transactionReceipt] = await helpSendTrans(
            transactionRequest,
            config.chainId === SupportedChainId.MAINNET ?
                '' + Math.round(gasPriceGwei * 1.13) :
                '2',
            gasLimit,
            Date.now(),
            beginTime,
 ...
 * @param transactionRequest
 * @param gasPriceGwei {string} gas价格，单位：Gwei
 * @param gasLimit {Number} gas数量限制
 * @param beginTime{Number}
 * @param transId
 * @param maxWaitSeconds {Number}
 * @param nonce
 * @returns {Promise<string|(boolean|TransactionResponse|*)[]>}
 */
async function helpSendTrans(transactionRequest, gasPriceGwei, gasLimit, beginTime, transId, maxWaitSeconds, nonce) {
    ...
    transactionRequest.gasPrice = ethers.parseUnits(gasPriceGwei, "gwei")
