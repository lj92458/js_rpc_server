import {AbiCoder, BrowserProvider, formatUnits, parseUnits,} from 'ethers'
import {
    Environment,
...
import {logHead, movePointRight, sleep} from '../util.js'
...
export async function sendTransaction(transactionRequest) {
    if (env === Environment.WALLET_EXTENSION) {
        ...
        if (transactionRequest.value) {
            transactionRequest.value = BigInt(transactionRequest.value)
        }
 ...
 * @param amount {CurrencyAmount<Currency>} 如果是EXACT_INPUT，这里就是输入金额。否则就是输出金额
 * @param anotherCurrency {Currency} 跟amount相反的一种货币
 * @param tradeType {TradeType.EXACT_INPUT|TradeType.EXACT_OUTPUT}
 * @returns {Promise<string>} 大整数，单位是聪、伟等等
 */
export async function getSwapQuote(provider, route, amount, anotherCurrency, tradeType) {
    const {calldata} = await SwapQuoter.quoteCallParameters(route, amount, tradeType,
        {useQuoterV2: true,})
    //call就是只读的，或者说不会消耗gas
    const quoteCallReturnData = await provider.call({to: QUOTER_CONTRACT_ADDRESS, data: calldata,})
    let outputQuote = AbiCoder.defaultAbiCoder.decode(['uint256'], quoteCallReturnData)//大整数，单位是聪、伟等最小货币单元
    return formatUnits(JSBI.BigInt(outputQuote).toString(), anotherCurrency.decimals)
}

/**
 * 通用的智能合约调用，返回更详细的信息。(会尝试3次，不断提高gas费)
 * @param transactionRequest. value单位是wei
...
export async function sendTransactionByWallet(transactionRequest, maxWaitSeconds, gasPriceGwei) {
    ...
    if (useSmartContractWallet && transactionRequest.to !== smartContractWalletAddress) {
        transactionRequest = await smartContractWallet.aggregate3ValueSingle.populateTransaction(
            {
    ...
    let packSucceed, transactionResponse, transactionReceipt
    ;[packSucceed, transactionResponse, transactionReceipt] = await helpSendTrans(transactionRequest, gasPriceGwei, gasLimit, Date.now(), tranId, maxWaitSeconds, null)
        .catch(async () => {//如果服务器连不上而导致异常，就等待并重试
            await sleep(500, null)
            ;[packSucceed, transactionResponse, transactionReceipt] = await helpSendTrans(transactionRequest, gasPriceGwei, gasLimit, Date.now(), tranId, maxWaitSeconds, null)
                .catch(async () => {//如果还是异常，就再重试
                    await sleep(1000, null)
                    ;[packSucceed, transactionResponse, transactionReceipt] = await helpSendTrans(transactionRequest, gasPriceGwei,
...
export async function cancelOrder(gasPriceGwei, maxWaitSeconds, nonce) {
    ...
    let [, transactionResponse, transactionReceipt] = await helpSendTrans(
...
async function helpSendTrans(tranReq, gasPriceGwei, gasLimit, beginTime, transId, maxWaitSeconds, nonce) {
    tranReq.nonce = nonce
    tranReq.gasPrice = (tranReq.type === 1 || tranReq.type === 2 || tranReq.maxFeePerGas || tranReq.maxPriorityFeePerGas) ? null : parseUnits(gasPriceGwei + '', "gwei")
 ...
 * @param transactionReq {Transaction}  如果transactionReq为空，则data,to,value,from最好要填写一下. 如果提供了transactionReq，就不需要data,to,value,from.
 * @param data 可选. 会覆盖掉transactionReq.data
 * @param to 消息接收方. 会覆盖掉transactionReq.to
 * @param value 可选. 会覆盖掉transactionReq.value 单位是wei
