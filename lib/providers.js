...
export async function getSwapQuote(provider, route, amount, anotherCurrency, tradeType) {
    ...
    let outputQuote = utils.defaultAbiCoder.decode(['uint256'], quoteCallReturnData)//大整数，单位是聪、伟等最小货币单元
    return utils.formatUnits(JSBI.BigInt(outputQuote).toString(), anotherCurrency.decimals)
}
