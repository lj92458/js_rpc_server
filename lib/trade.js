...
export async function createTrade(provider, tokenIn, tokenOut, amountIn, needAmountOut, poolFee, slippage) {
    ...
    let decimalOutAmount = ethers.utils.formatUnits(outputQuote, tokenOut.decimals)
    ...
    return Trade.createUncheckedTrade({
        route: swapRoute,
        inputAmount: CurrencyAmount.fromRawAmount(tokenIn, ethers.utils.parseUnits(amountIn.toString(), tokenIn.decimals).toString()),
        outputAmount: CurrencyAmount.fromRawAmount(tokenOut, ethers.utils.parseUnits(needAmountOut.toString(), tokenOut.decimals).toString()),
...
async function getOutputQuote(provider, route, tokenIn, amountIn) {
    const {calldata} = await SwapQuoter.quoteCallParameters(
        route,
        CurrencyAmount.fromRawAmount(tokenIn, ethers.utils.parseUnits(amountIn.toString(), tokenIn.decimals).toString()),
