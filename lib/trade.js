...
export async function createTrade(provider, tokenIn, tokenOut, amountIn, needAmountOut, poolFee, slippage) {
    //计算要提交的outputAmount，它应该扣除滑点
    let inputAmount = CurrencyAmount.fromRawAmount(tokenIn, utils.parseUnits(amountIn.toString(), tokenIn.decimals).toString())
    let outputAmount = CurrencyAmount.fromRawAmount(tokenOut, utils.parseUnits((needAmountOut * (1 - slippage)).toString(), tokenOut.decimals).toString())
    let pool = await creatPoolWithticksFromPool(await getPool(provider, tokenIn, tokenOut, poolFee))
    ...
    let poolOut = (await pool.getOutputAmount(inputAmount))[0].toFixed(9)
    let actualSlippageFromPool = ((needAmountOut - poolOut) / needAmountOut * 100).toFixed(2)//自己从pool.getOutputAmount计算出的滑点
    let info = `期望滑点${slippage * 100}%，实际滑点${actualSlippage}%，自己从pool.getOutputAmount计算的滑点${actualSlippageFromPool}%`
