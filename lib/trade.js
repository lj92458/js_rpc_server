...
export async function createTrade(provider, tokenIn, tokenOut, amountIn, needAmountOut, poolFee, slippage) {
    ...
    let inputAmount = CurrencyAmount.fromRawAmount(tokenIn, movePointRight(amountIn.toString(), tokenIn.decimals))
    let outputAmount = CurrencyAmount.fromRawAmount(tokenOut, movePointRight(needAmountOut.toString(), tokenOut.decimals))
    ...
    let poolOut = (await pool2.getOutputAmount(inputAmount))[0].toFixed()
...
async function getOutputQuote(provider, route, tokenIn, amountIn) {
    const {calldata} = await SwapQuoter.quoteCallParameters(
        route,
        CurrencyAmount.fromRawAmount(tokenIn, movePointRight(amountIn.toString(), tokenIn.decimals)),
...
export async function getTokenTransferApproval(token, amount, maxWaitSecond, gasPriceGwei) {
    ...
    try {
        ...
        const transaction = await tokenContract.populateTransaction.approve(
            SWAP_ROUTER_ADDRESS,
            movePointRight(amount.toString(), token.decimals)
        )
