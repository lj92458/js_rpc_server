...
import {QUOTER_CONTRACT_ADDRESS, SWAP_ROUTER_ADDRESS, IERC20, TOKEN_AMOUNT_TO_APPROVE_FOR_TRANSFER} from './constant.js'
import {getProvider, getWalletAddress, sendTransactionByWallet, TransactionState, wallet} from './providers.js'
...
import {doubleToPersent, logHead} from '../util.js'
 ...
 * @param slippage {Number|string} 能容忍的滑点. 0.001表示0.1%
...
export async function createTrade(provider, tokenIn, tokenOut, amountIn, needAmountOut, poolFee, slippage) {
    ...
    return Trade.createUncheckedTrade({
        route: swapRoute,
        inputAmount: CurrencyAmount.fromRawAmount(tokenIn, utils.parseUnits(amountIn.toString(), tokenIn.decimals).toString()),
        outputAmount: CurrencyAmount.fromRawAmount(tokenOut, utils.parseUnits(needAmountOut.toString(), tokenOut.decimals).toString()),
