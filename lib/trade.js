...
import {fillTranRequest, getProvider, getWalletAddress, sendTransactionByWallet, TransactionState} from './providers.js'
...
export async function createTrade(provider, tokenIn, tokenOut, amountIn, needAmountOut, poolFee, slippage) {
    ...
    let pool1 = await getPool(provider, tokenIn, tokenOut, poolFee, false) //实时获取pool,因为市场剧烈波动，价格已经变了
...
async function getOutputQuote(provider, route, tokenIn, amountIn) {
    const {calldata} = await SwapQuoter.quoteCallParameters(
        route,
        CurrencyAmount.fromRawAmount(tokenIn, movePointRight(amountIn, tokenIn.decimals)),
